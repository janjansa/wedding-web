...
//"gulp" basic command
//npm i postcss@8.1.0 - fixed postcss problems
...
function scripts() {
	return gulp
		.src('src/js/*.js').on('error', console.log)
		.pipe(sourcemaps.init())
		.pipe(include())
		.pipe(sourcemaps.write("."))
...
function watchFiles() {
  ...
  gulp.watch('src/scss/print.scss', gulp.series(cssPrint));
	...
	gulp.watch(
	['build/*.css', 'build/*.html', 'build/js/*.js', themePath + '*.php', themePath + '**/*.php'],
